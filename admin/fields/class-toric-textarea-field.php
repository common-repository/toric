<?php
/**
 * Contains Toric_Textarea_Field class.
 *
 ...
 * @since 1.0.0
 */
class Toric_Textarea_Field implements Toric_Fields {
	/**
	...
	/**
	 * Constructors Toric_Textarea_Field
	 *
	 * @since 1.0.0
	 *
	...
	public function render( WP_Post $post, Toric_Meta_Box $meta_box ) {
		...
		<div>
		<textarea id="<?php echo esc_attr( $field_id ); ?>" name="<?php echo esc_attr( $field_id ); ?>" rows="18" cols="50" class="<?php echo esc_attr( $class ); ?>" placeholder="<?php echo esc_attr( $placeholder ); ?>" id="editing" spellcheck="false"><?php echo esc_attr( $this->get_database_value( $post->ID ) ); ?></textarea>
		</div>
		<?php
	}
