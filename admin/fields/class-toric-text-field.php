 ...
 * @since 1.0.0
 */
class Toric_Text_Field implements Toric_Fields {
	/**
	...
	public function render( WP_Post $post, Toric_Meta_Box $meta_box ) {
		...
		?>
		<div class="toric field section">
		<label for="<?php echo esc_attr( $field_id ); ?>">
			<?php echo esc_attr( $this->description ); ?>
		</label>
		<input class="<?php echo esc_attr( $class ); ?>" type="text" id="<?php echo esc_attr( $field_id ); ?>" name="<?php echo esc_attr( $field_id ); ?>"   placeholder="<?php echo esc_attr( $placeholder ); ?>" value="<?php echo esc_attr( $this->get_database_value( $post->ID ) ); ?>" oninput="toricContentOnInput(this.value)">
		</div>
		<?php
	}
